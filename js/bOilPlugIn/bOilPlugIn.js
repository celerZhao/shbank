...
bOilPlugIn.prototype = {
            ...
            if (thisVal == '') {
                ...
                if (!_this.regs[argCurr].reg.test(thisVal)) {
                    ...
                    if (argCurr == 'reDealPassword') {//如果是重复密码校验
                        if ($('#' + argCurr).val() != $('#dealPassword').val()) {//判断两次输入的支付密码是否相同
                            alertMsgFun(_this);
                        }
                    }

                    //重复加油卡号码
                    if (argCurr == 'reOilCard') {//如果是重复加油卡号码校验
                        if ($('#' + argCurr).val() != $('#oilCard').val()) {//判断两次输入的加油卡号码是否相同
                            alertMsgFun(_this);
                        }
                    }
                }
    ...
    tips: function (config) {
        var _this = this, 
        o = {
            btnEl:config.btnEl||null,  // 触发弹框的按钮
            title: config.title || '',
    ...
    // 触发按钮增加active属性，防止重复点击
        if(o.btnEl != null){
            if( o.btnEl.hasClass("active")){
                return
            }else{
                o.btnEl.addClass("active");
            }
        }
        if (!o.alone) {
        ...
        if (o.btnNums === 1) {
            ...
            // 配置按钮颜色 btnClass
            var cancle_btn_class =( o.btn.btnCancle.btnClass== undefined || o.btn.btnCancle.btnClass=="")? "btn-cancle":o.btn.btnCancle.btnClass;
            btnHtml = '<div class="btnCont clear">' +
                '<span class="left"><a id="btnCancle" class="btn '+cancle_btn_class+'" href="' + o.btn.btnCancle.href + '">' + o.btn.btnCancle.text + '</a></span>' +
        ...
        $('#btnSure').on('click', function () {
            if (typeof o.btnSureCallback == 'function') {
                var e = $(this), resInput = o.btnSureCallback();
                if (typeof resInput == 'function') {//如果回调函数返回一个函数
                    resInput(e);
                } else if (resInput === 'loading'){
                    //等候加载提示
    ...
    // 2s 之后，清除active 保证下一次按钮可以点击
        setTimeout(function(){
            o.btnEl.removeClass("active");
        },2000)
    },
